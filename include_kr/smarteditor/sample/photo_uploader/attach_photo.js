    	...
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		...
    		try{
	    		if(!!tempFile){
	    			...
	    			callAjaxForHTML5(tempFile, k, sUploadURL);
	    			k += 1;
	    		}
	    	}catch(e){}
    		tempFile = null;
    ...
    function callAjaxForHTML5 (tempFile, k, sUploadURL){
    	var temp = new Date();
    	var nName = temp.getFullYear() + "" + (temp.getMonth()+1) + "" + temp.getDate() + "" + temp.getHours() + "" + temp.getMinutes() + "" + temp.getSeconds() + "" + k + "." + (tempFile.name.split('.').pop());
