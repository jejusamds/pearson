					...
					<table class="table">
						...
						<tbody>
							...
							?>
								<tr>
									<td><?=$rowNo?></td>
									<td><span class="ellipsis">(<?=$cfg['board']['category'][$_GET['board']][$row['category']]?>) <?=$row['subject']?></span></td>
									<td><span class="ellipsis"><?=$row['name']?></span></td>
									<td><span class="ellipsis"><?=decodeData($row['tmp1'])?></span></td>
								...
								<? $rowNo--;?>
							<? } ?>
							<? if(count($rs) < 1){ echo "<tr><td colspan='7' class='text-center'>No Data</td></tr>"; } ?>
						</tbody>
					</table>
				</div>
